...
function StudyMaterial() {
  ...
  // Search dynamically across every already-loaded class → subject → chapter → file,
  // not just the currently selected class. A match at a higher level (class/subject/
  // chapter name) pulls in everything nested under it; a match on a file name alone
  // only pulls in that file.
  const searchResults: StudySearchResult[] = isSearching
    ? classes.flatMap((cls) => {
        const classMatches = normalizeSearchText(cls.name).includes(normalizedQuery);
        return (cls.subjects ?? []).flatMap((subject) => {
          const subjectMatches =
            classMatches || normalizeSearchText(subject.name).includes(normalizedQuery);
  ...
  const activeDisplaySubject =
    displaySubjects?.find((s) => s.id === activeSubject) || displaySubjects?.[0];
                  ...
                  <div className="sm-chapters-list">
                    ...
                    {searchResults.length === 0 ? (
                      <div className="sm-empty-state">
                        ...
                        <span>
                          Try a different class, subject, chapter or file name.
                        </span>
                      </div>
                    ) : (
